...
bayesian_result = stan_inference(prob1,t,data,priors;
    num_samples=1000, num_warmups=1000, num_chains=4, likelihood=Normal)

bayesian_result |> display

@test mean(get(bayesian_result.chains,:theta_1)[1]) ≈ 1.5 atol=3e-1

# Test norecompile
println("\nOne parameter case - test if compiled again\n")
...
println("\nOne parameter case - truncated priors\n")
priors = [truncated(Normal(1.,0.01),0.5,2.0),truncated(Normal(1.,0.01),0.5,2.0),truncated(Normal(1.5,0.01),1.0,2.0)]
bayesian_result = stan_inference(prob1,t,data,priors;likelihood=Normal,sample_u0=true)
...
bayesian_result = stan_inference(prob1,t,data,priors;likelihood=Normal,save_idxs=[1])
...
bayesian_result = stan_inference(prob1,t,data,priors;
    likelihood=Normal,save_idxs=[1],sample_u0=true)
