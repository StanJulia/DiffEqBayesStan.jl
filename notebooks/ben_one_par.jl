...
# ╔═╡ 6431b2e2-46cf-47e9-ab98-e867585deec3
sm = SampleModel("ben_one_par", ben_model)

# ╔═╡ 4c31edf6-8dd8-4b74-89a4-17144308967d
obs = reshape([
...
# ╔═╡ b9d1f5f5-61c2-4045-a343-2ea12a5f3ec4
ben_data = Dict(
  :N => 10,
  :t0 => 0,
  :ts => 1:10,
...
# ╔═╡ f13761e1-16ca-4484-8667-7ad7628c5f82
begin
	t = ben_data[:ts]
...
# ╔═╡ ef6479d5-9388-4d8a-bd30-ee67029824b5
@time rc = stan_sample(sm; data=ben_data);

# ╔═╡ 36b1dc7b-f406-4bc0-a57b-d3fd29890f77
if success(rc)
  df = read_samples(sm, :dataframe)
end

# ╔═╡ 36cd07da-3969-4fb7-9fcb-1859523e6882
let
	if success(rc)

		# Uncomment for local chain inspection
		p1 = Vector{Plots.Plot{Plots.GRBackend}}(undef, 1)
		p2 = Vector{Plots.Plot{Plots.GRBackend}}(undef, 1)
		for i in 1:1
			p1[i] = plot(title="theta.$i")
			plot!(p1[i], df[!, Symbol("theta.$i")])
			p2[i] = plot(title="theta.$i")
			density!(p2[i], df[!, Symbol("theta.$i")])
		end
		plot(p1..., p2..., layout=(1, 2))
	end
end

# ╔═╡ 51a7225e-638f-4c69-a01e-0061db65d083
...
git-tree-sha1 = "81b5b4d5474e21ea42211482728e1e8cf7bb75fe"
...
[[deps.StanSample]]
deps = ["CSV", "DataFrames", "DelimitedFiles", "Distributed", "DocStringExtensions", "MonteCarloMeasurements", "NamedTupleTools", "OrderedCollections", "Parameters", "Random", "Requires", "StanBase", "StanDump", "TableOperations", "Tables", "Unicode"]
git-tree-sha1 = "87e39f49454ff8023d7135a7f9a4fec4f3b58c9d"
